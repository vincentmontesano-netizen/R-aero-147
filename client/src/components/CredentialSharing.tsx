...
export default function CredentialSharing({data}:{data:Dossier}){
 ...
 return <section className="rounded-xl border bg-card p-4 space-y-4" aria-labelledby="credential-sharing-title">
  ...
  {data.orgViews.length===0?<p className="text-sm">{w.noCompany}</p>:valid.length===0?<p className="text-sm">{w.noCertificate}</p>:<form className="grid gap-3 sm:grid-cols-2" onSubmit={e=>{e.preventDefault();if(!busy&&orgId&&certificateId)share.mutate({orgId:Number(orgId),certificateId:Number(certificateId)});}}>
   <label className="text-sm space-y-1"><span>{w.certificate}</span><select required value={certificateId} disabled={busy} onChange={e=>setCertificateId(e.target.value)} className="w-full min-w-0 rounded-md border bg-card p-2"><option value="">{w.choose}</option>{valid.map(c=><option key={c.id} value={c.id}>{data.privateCredentials.find(p=>p.certificateId===c.id)?.displayTitle??c.certificateNumber} · {c.certificateNumber}</option>)}</select></label>
   <label className="text-sm space-y-1"><span>{w.company}</span><select required value={orgId} disabled={busy} onChange={e=>setOrgId(e.target.value)} className="w-full min-w-0 rounded-md border bg-card p-2"><option value="">{w.choose}</option>{data.orgViews.map(o=><option key={o.orgId} value={o.orgId}>{o.orgName??`#${o.orgId}`}</option>)}</select></label>
