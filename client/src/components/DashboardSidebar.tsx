import { useState } from "react";
import type { LucideIcon } from "lucide-react";
import { PanelLeft } from "lucide-react";
import { useI18n } from "@/i18n";

const BLUE = "var(--foreground)";
const GOLD = "var(--link)";
const MUTED = "var(--muted-foreground)";
const BORDER = "var(--border)";

export type SidebarItem = {
  key: string;
  label: string;
  icon: LucideIcon;
  badge?: number | string;
  section?: string;
};

/** Responsive navigation for the connected dashboards.
 *  - Desktop (md+): vertical, sticky, collapsible sidebar (icons-only when collapsed).
 *  - Mobile (<md): a full-width horizontal scrollable bar of pills above the content. */
export default function DashboardSidebar({
  items,
  active,
  onSelect,
  heading,
}: {
  items: SidebarItem[];
  active: string;
  onSelect: (key: string) => void;
  heading?: string;
}) {
  const { t } = useI18n();
  const [collapsed, setCollapsed] = useState(false);
  return (
    <aside
      className={`w-full md:self-start md:sticky md:top-20 rounded-xl overflow-hidden md:transition-[width] md:duration-200 ${collapsed ? "md:w-14" : "md:w-56"}`}
      style={{
        background: "var(--card)",
        border: `1px solid ${"var(--border)"}`,
      }}
    >
      {/* Header + collapse toggle — desktop only */}
      <div
        className="hidden md:flex items-center gap-2 p-2"
        style={{ borderBottom: `1px solid ${"var(--border)"}` }}
      >
        {!collapsed && heading && (
          <span
            className="text-xs font-semibold tracking-widest px-1.5 truncate flex-1"
            style={{ color: "var(--muted-foreground)" }}
          >
            {heading}
          </span>
        )}
        <button
          onClick={() => setCollapsed(c => !c)}
          title={t(collapsed ? "sidebar.expand" : "sidebar.collapse")}
          aria-label={t(collapsed ? "sidebar.expand" : "sidebar.collapse")}
          aria-expanded={!collapsed}
          className="p-1.5 rounded-md hover:bg-foreground/5 ml-auto shrink-0"
          style={{ color: "var(--muted-foreground)" }}
        >
          <PanelLeft className="w-4 h-4" />
        </button>
      </div>
      <nav
        aria-label={heading}
        className="flex md:flex-col gap-1 p-2 overflow-x-auto md:overflow-visible"
      >
        {items.map((it, i) => {
          const on = active === it.key;
          const showSection =
            !!it.section && it.section !== items[i - 1]?.section;
          return (
            <div key={it.key} className="contents">
              {showSection && !collapsed && (
                <div
                  className="hidden md:block text-xs font-semibold tracking-widest px-2 pt-3 pb-1 first:pt-1"
                  style={{ color: "var(--muted-foreground)" }}
                >
                  {it.section}
                </div>
              )}
              <button
                type="button"
                aria-pressed={on}
                onClick={() => onSelect(it.key)}
                title={it.label}
                className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm font-medium transition-colors shrink-0 md:w-full justify-start ${collapsed ? "md:justify-center" : ""}`}
                style={{
                  background: on
                    ? "color-mix(in srgb, var(--link) 13%, transparent)"
                    : "transparent",
                  color: on ? "var(--foreground)" : "var(--muted-foreground)",
                  border: on
                    ? "1px solid color-mix(in srgb, var(--link) 33%, transparent)"
                    : "1px solid transparent",
                }}
              >
                <it.icon
                  className="w-4 h-4 shrink-0"
                  style={{
                    color: on ? "var(--link)" : "var(--muted-foreground)",
                  }}
                />
                <span
                  className={`truncate text-left ${collapsed ? "md:hidden" : "md:flex-1"}`}
                >
                  {it.label}
                </span>
                {it.badge != null && (
                  <span
                    className={`text-xs px-1.5 rounded-full ${collapsed ? "md:hidden" : ""}`}
                    style={{
                      background:
                        "color-mix(in srgb, var(--link) 13%, transparent)",
                      color: "var(--foreground)",
                    }}
                  >
                    {it.badge}
                  </span>
                )}
              </button>
            </div>
          );
        })}
      </nav>
    </aside>
  );
}
