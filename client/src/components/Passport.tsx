              ...
              return (
                <div key={e.id} className="p-3 rounded-lg" style={{ background: "oklch(97% 0.01 88)" }}>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: "oklch(92% 0.01 240)", color }}>
                  ...
                  {cert && (
                    <div className="flex items-center gap-3 mt-2 pt-2 pl-7" style={{ borderTop: `1px dashed ${BORDER}` }}>
                      <div className="flex-1 min-w-0 text-xs truncate" style={{ color: MUTED }}>
                        ...
                        {cert.expiresAt && <> · {t("passport.expires")} <span style={{ color: expired ? RED : MUTED }}>{new Date(cert.expiresAt).toLocaleDateString(lang)}</span></>}
                      </div>
        ...
        <div className="space-y-3">
          {tags.map((rating) => {
            ...
            const status = {
              ...
              bg: expiry === 'expired' ? 'oklch(55% 0.22 27 / 0.1)' : 'oklch(92% 0.01 240)',
            ...
            return (
              <div key={rating} className="rounded-lg p-3" style={{ background: "oklch(97% 0.01 88)" }}>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full max-w-40" style={{ background: status.bg, color: status.fg }}>{status.label}</span>
                    ...
                    {docs.map((doc) => {
                      ...
                      return (
                        <div key={doc.id} className="flex items-center gap-2">
                          ...
                          <button onClick={() => { if (confirm(t("passport.confirmDelete"))) del.mutate({ id: doc.id }); }} className="text-red-500 p-1 shrink-0" title={t("passport.delete")}><Archive className="w-4 h-4" /></button>
                        </div>
                      );
                    })}
                  </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <fieldset disabled={submission.busy} aria-busy={submission.busy} className="space-y-3">
            ...
            <div>
              ...
              <input id={fileInputId} aria-describedby={`${fileInputId}-formats ${fileInputId}-hint`} type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => setForm((f) => ({ ...f, file: e.target.files?.[0] ?? null }))} className="w-full text-sm mt-1 file:mr-2 file:rounded file:border-0 file:px-2 file:py-1 file:text-xs file:cursor-pointer" style={{ color: MUTED }} />
              <p id={`${fileInputId}-formats`} className="text-xs mt-1">{t("passport.formats")}</p>
              <p id={`${fileInputId}-hint`} className="text-[11px] mt-1" style={{ color: MUTED }}>{t("passport.fileHint")}</p>
            </div>
            {submission.readError && <p role="alert" className="text-sm text-red-700">{t('passport.fileReadError')}</p>}
              {submission.sendError && <p role="alert" className="text-sm text-red-700">{t('passport.uploadUnconfirmed')}</p>}
            <Button disabled={submission.busy} onClick={upload} className="w-full" style={{ background: BLUE, color: "white" }}>{submission.busy ? t("passport.uploading") : t("passport.save")}</Button>
          </fieldset>
        </DialogContent>
      </Dialog>
    </div>
          ...
          <DialogContent className="max-w-md">
            ...
            <fieldset disabled={submission.busy} aria-busy={submission.busy} className="space-y-3">
              <div>
                ...
                <select id={kindInputId} value={form.kind} onChange={(e) => setForm((f) => ({ ...f, kind: e.target.value as Kind }))} className="w-full h-9 rounded-md border px-2 text-sm mt-1" style={{ borderColor: BORDER }}>
              ...
              <div>
                <Label htmlFor={fileInputId}>{t("passport.fieldFile")}</Label>
                <input id={fileInputId} aria-describedby={`${fileInputId}-formats ${fileInputId}-hint`} type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="w-full text-sm mt-1 file:mr-2 file:rounded file:border-0 file:px-2 file:py-1 file:text-xs file:cursor-pointer" style={{ color: MUTED }} />
              <p id={`${fileInputId}-formats`} className="text-xs mt-1">{t("passport.formats")}</p>
                <p id={`${fileInputId}-hint`} className="text-[11px] mt-1" style={{ color: MUTED }}>{t("passport.fileHint")}</p>
              </div>
              {submission.readError && <p role="alert" className="text-sm text-red-700">{t('passport.fileReadError')}</p>}
              {submission.sendError && <p role="alert" className="text-sm text-red-700">{t('passport.uploadUnconfirmed')}</p>}
              <Button disabled={submission.busy} onClick={submit} className="w-full" style={{ background: BLUE, color: "white" }}>{submission.busy ? t("passport.uploading") : t("passport.save")}</Button>
            </fieldset>
          </DialogContent>
        </Dialog>
      </div>
          ...
          {docs.map((doc) => {
            ...
            return (
              <div key={doc.id} className="flex items-center gap-3 p-3 rounded-lg" style={{ background: "oklch(97% 0.01 88)" }}>
                ...
                <button onClick={() => { if (confirm(t("passport.confirmDelete"))) del.mutate({ id: doc.id }); }} className="text-red-500 p-1.5 shrink-0" title={t("passport.delete")}><Archive className="w-4 h-4" /></button>
              </div>
            );
          })}
        </div>
...
function Label({ children, htmlFor }: { children: ReactNode; htmlFor?: string }) {
  return <label htmlFor={htmlFor} className="text-xs font-semibold" style={{ color: MUTED }}>{children}</label>;
}
...
function PassportArchives() {
  ...
  return <div className="rounded-2xl p-6 space-y-4" style={CARD}>
    ...
    <p className="text-sm" style={{ color: MUTED }}>{t("passport.archiveHint")}</p>
