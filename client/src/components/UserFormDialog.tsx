    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3 mt-1">
          <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelName")}</label><Input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder={t("userFormDialog.placeholderName")} /></div>
          {mode === "new" && (
            <>
              <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelEmail")}</label><Input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="jean@example.com" /></div>
              <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelPassword")}</label><Input type="text" value={form.password} onChange={(e) => set("password", e.target.value)} placeholder={t("userFormDialog.placeholderPassword")} /></div>
            </>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div className={form.role === "admin" ? "col-span-2" : ""}>
              <label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelRole")}</label>
              <select value={form.role} onChange={(e) => set("role", e.target.value)} className="w-full h-9 rounded-md border px-2 text-sm" style={{ borderColor: BORDER }}>
                {ROLES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            {form.role !== "admin" && (
              <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelJobTitle")}</label><Input value={form.jobTitle} onChange={(e) => set("jobTitle", e.target.value)} placeholder={t("userFormDialog.placeholderJobTitle")} /></div>
            )}
          </div>
          {/* Part-66 / job fields are irrelevant for an admin account. */}
          {form.role !== "admin" && (
            <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.labelLicense")}</label><Input value={form.licenseNumber} onChange={(e) => set("licenseNumber", e.target.value)} placeholder="FR.66.XXXXXXXX" className="font-mono" /></div>
          )}

          {/* Optionally create an organisation and make this user its manager. */}
          {mode === "new" && (
            <div className="rounded-lg p-3" style={{ border: `1px solid ${BORDER}`, background: "oklch(97% 0.01 88)" }}>
              <label className="flex items-center justify-between gap-3 cursor-pointer">
                <span className="text-sm font-medium" style={{ color: BLUE }}>{t("userFormDialog.createOrgToggle")}</span>
                <input type="checkbox" checked={form.createOrg} onChange={(e) => setForm((f) => ({ ...f, createOrg: e.target.checked }))} className="w-4 h-4" />
              </label>
              {form.createOrg && (
                <div className="space-y-2 mt-3">
                  <p className="text-[11px]" style={{ color: MUTED }}>{t("userFormDialog.createOrgHint")}</p>
                  <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.orgNameLabel")}</label><Input value={form.orgName} onChange={(e) => set("orgName", e.target.value)} placeholder="Aero MRO SA" /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.orgTypeLabel")}</label>
                      <select value={form.orgType} onChange={(e) => set("orgType", e.target.value)} className="w-full h-9 rounded-md border px-2 text-sm" style={{ borderColor: BORDER }}>
                    ...
                    <div><label className={lbl} style={{ color: MUTED }}>{t("userFormDialog.orgAgreementLabel")}</label><Input value={form.orgAgreement} onChange={(e) => set("orgAgreement", e.target.value)} placeholder="FR.145.XXXX" className="font-mono" /></div>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <Button onClick={save} disabled={create.isPending || update.isPending} style={{ background: BLUE, color: "white" }}>{t("userFormDialog.save")}</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
