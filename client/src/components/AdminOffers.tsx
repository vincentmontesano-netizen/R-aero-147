  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
        <h2 className="font-semibold" style={{ color: "var(--foreground)" }}>{t("adminOffers.title")}</h2>
        <div className="flex gap-1 rounded-lg p-0.5" style={{ background: "var(--muted)" }}>
          {["fr", "en"].map((l) => (
            <button key={l} onClick={() => { setLang(l); setDraft(blank(l)); }} className="text-sm px-3 py-1 rounded-md font-medium" style={lang === l ? { background: "var(--card)", color: "var(--foreground)" } : { color: "var(--muted-foreground)" }}>{l.toUpperCase()}</button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl p-4 mb-5 max-w-2xl" style={{ background: "var(--card)", border: `1px solid ${"var(--border)"}` }}>
        <div className="text-sm font-semibold mb-2" style={{ color: "var(--foreground)" }}>{t("adminOffers.newOffer")}</div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <div className="sm:col-span-2">
            <label className="text-sm" style={{ color: "var(--muted-foreground)" }}>{t("adminOffers.features")}</label>
        ...
        <Button size="sm" className="mt-3" disabled={!draft.name.trim() || create.isPending} onClick={() => create.mutate({ ...draft, features: toArr(draft.features) })} style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}><Plus className="w-4 h-4 mr-1" /> {t("adminOffers.add")}</Button>
      </div>

      {/* Existing */}
      <div className="space-y-2 max-w-2xl">
        {(offers as any[]).length === 0 && <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>{t("adminOffers.empty")}</p>}
        {(offers as any[]).map((o) => (
          <div key={o.id} className="rounded-xl p-4" style={{ background: "var(--card)", border: `1px solid ${"var(--border)"}` }}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="flex items-center gap-3">
                <button onClick={() => update.mutate({ id: o.id, highlight: !o.highlight })} className="flex items-center gap-1 text-sm" style={{ color: o.highlight ? "var(--link)" : "var(--muted-foreground)" }}><Star className="w-3.5 h-3.5" fill={o.highlight ? "currentColor" : "none"} /> {t("adminOffers.highlight")}</button>
                <button onClick={() => update.mutate({ id: o.id, isActive: !o.isActive })} className="text-sm" style={{ color: o.isActive ? "var(--success)" : "var(--muted-foreground)" }}>{o.isActive ? t("adminOffers.active") : t("adminOffers.inactive")}</button>
              </div>
              <button onClick={() => { if (confirm(t("adminOffers.confirmDelete"))) del.mutate({ id: o.id }); }} className="text-destructive p-1"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
