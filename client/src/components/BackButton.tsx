...
export default function BackButton({ fallback = "/", dark = false, label }: { fallback?: string; dark?: boolean; label?: string }) {
  ...
  return (
    <button
      onClick={goBack}
      className={`inline-flex items-center gap-1.5 text-sm font-medium mb-4 transition-colors ${dark ? "text-muted-foreground hover:text-foreground" : "hover:opacity-70"}`}
