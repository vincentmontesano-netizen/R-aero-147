  ...
  return (
    <div>
      {query.isError?<div><p role="alert" className="text-sm">{t("quoteThread.unavailable")}</p><Button variant="outline" disabled={query.isFetching} onClick={()=>void query.refetch()}>{t("quoteThread.retry")}</Button></div>:query.isPending?<p role="status">{t("common.loading")}</p>:<div className="space-y-2 max-h-72 overflow-y-auto mb-3 pr-1">
        {msgs.length === 0 && <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>{t("quoteThread.empty")}</p>}
        {msgs.map((m: any) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
              <div className="rounded-lg px-3 py-2 max-w-[80%]" style={{ background: mine ? "color-mix(in srgb, var(--link) 13%, transparent)" : "var(--background)" }}>
                <div className="text-xs mb-0.5" style={{ color: "var(--muted-foreground)" }}>{m.fromName ?? "—"}{m.fromRole === "admin" ? " · R-AERO" : ""} · {new Date(m.createdAt).toLocaleString(lang,{dateStyle:"short",timeStyle:"short"})}</div>
      ...
      <div className="flex gap-2">
        ...
        <Button aria-label={t("quoteThread.send")} disabled={!text.trim() || send.isPending || query.isError || query.isPending} onClick={submit} style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
