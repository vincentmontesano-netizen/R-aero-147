...
function VerificationWorkspace() {
  ...
  const inputStyle = "rounded-md border border-input bg-card p-3 min-h-11 w-full";
  const uploadErrors = {
  ...
  return <div className="min-h-screen bg-muted text-foreground"><PublicNav /><main className="max-w-7xl mx-auto px-5 pt-28 pb-20">
    ...
    <header className="flex flex-wrap justify-between items-start gap-6 mb-8"><div><p className="text-sm tracking-widest text-warning mb-3">KYC / KYB</p><h1 className="text-3xl font-sans font-semibold flex items-center gap-3"><ShieldCheck />{tr("Vérification des dossiers", "Document verification", "التحقق من الملفات")}</h1><p className="text-muted-foreground mt-4 max-w-2xl">{tr("Une revue documentaire par notre équipe. Vos justificatifs restent privés. Cette validation ne constitue pas un agrément aéronautique.", "A documentary review by our team. Your evidence stays private. This validation is not an aviation approval.", "مراجعة وثائق يجريها فريقنا. تبقى مستنداتك خاصة. هذا التحقق لا يمثل اعتمادًا للطيران.")}</p></div>
    ...
    <div className="grid lg:grid-cols-[300px_1fr] gap-7"><aside className="space-y-3">
      ...
      {queue && <div className="space-y-3 rounded-xl border bg-card p-4">
        ...
        <p className="text-xs text-muted-foreground">{tr("50 dossiers par page, du plus récemment créé au plus ancien.", "50 files per page, newest created first.", "50 ملفًا في الصفحة، بدءًا بالأحدث إنشاءً.")}</p>
      ...
      {!rows.isLoading && !rows.isError && entries?.length === 0 && <p className="p-5 text-muted-foreground">{tr("Aucun dossier dans cette liste.", "No files in this list.", "لا توجد ملفات في هذه القائمة.")}</p>}
      {!rows.isError && entries?.map(item => <button key={item.id} disabled={busy} onClick={() => { if (selected === item.id || !canLeave()) return; resetDraft(); setNote(""); setSelected(item.id); }} className={`w-full text-start p-5 rounded-xl border ${selected === item.id ? "border-input bg-muted" : "border-border bg-card"}`}><span className="text-xs tracking-widest">{item.kind.toUpperCase()} · #{item.id}</span><strong className="block my-2">{item.legalName}</strong><span className="text-sm text-muted-foreground">{labels[item.status]}</span></button>)}
      ...
      {detail.isError ? <VerificationReadError busy={detail.isFetching} retry={() => { void detail.refetch(); }} /> : selected && detail.isLoading ? <p role="status">{tr("Chargement…", "Loading…", "جار التحميل…")}</p> : queue && !record ? <p className="p-10 rounded-xl border bg-card">{tr("Sélectionnez un dossier à examiner.", "Select a file to review.", "اختر ملفًا للمراجعة.")}</p> : <>
      {record?.reviewNote && <div className="p-5 rounded-xl bg-warning/10 border border-warning/30"><strong>{labels[record.status]}</strong><p className="mt-2 whitespace-pre-wrap">{record.reviewNote}</p></div>}
      {save.error?.data?.code==='CONFLICT'&&<p role="alert" className="p-4 rounded border border-warning/30 bg-warning/10">{tr("La sauvegarde a été refusée car le dossier a changé. Si vous souhaitez conserver vos saisies, copiez-les avant de rouvrir le dossier.","Saving was refused because the file has changed. If you wish to keep your entries, copy them before reopening the file.","تم رفض الحفظ لأن الملف تغيّر. إذا أردت الاحتفاظ بمدخلاتك، فانسخها قبل إعادة فتح الملف.")}</p>}
      <form className="p-6 md:p-8 bg-card rounded-xl border border-border space-y-5" onSubmit={e => { e.preventDefault(); if (!editable || busy || organizationsUnavailable) return; save.mutate({ ...form, expectedRevision:loadedRevision.current, companyId: form.kind === "kyb" ? Number(form.companyId) : undefined }); }}>
      ...
      {record && <section className="p-6 md:p-8 bg-card rounded-xl border space-y-5"><h2 className="font-sans text-xl font-semibold">{tr("Justificatifs", "Evidence", "المستندات")}</h2><p className="text-sm text-muted-foreground">{record.kind === "kyc" ? tr("Requis : une pièce d’identité. PDF, JPEG ou PNG, 8 Mo maximum.", "Required: an identity document. PDF, JPEG or PNG, up to 8 MB.", "المطلوب: وثيقة هوية. PDF أو JPEG أو PNG حتى 8 ميغابايت.") : tr("Requis : immatriculation de la société et mandat du représentant. PDF, JPEG ou PNG, 8 Mo maximum.", "Required: company registration and representative authority. PDF, JPEG or PNG, up to 8 MB.", "المطلوب: تسجيل الشركة وتفويض الممثل. PDF أو JPEG أو PNG حتى 8 ميغابايت.")}</p>
        ...
        {uploadProblem && <p role="alert" className="text-sm text-destructive">{uploadErrors[uploadProblem]}</p>}
        {editable && hasChanges && <p role="status" className="text-sm text-warning">{tr("Enregistrez vos modifications avant l’envoi pour examen.", "Save your changes before submitting for review.", "احفظ التغييرات قبل الإرسال للمراجعة.")}</p>}
...
function VerificationReadError({ busy, retry }: { busy: boolean; retry: () => void }) {
  ...
  return <div className="space-y-3 rounded-xl border border-warning/30 bg-warning/10 p-5">
...
function VerificationHistory({id,labels}:{id:number;labels:Record<string,string>}) {
  ...
  return <section className="p-6 bg-card rounded-xl border space-y-4">
    <h2 className="font-sans text-xl font-semibold flex items-center gap-2"><History size={20}/>{tr('Historique','History','السجل')}</h2>
    <p className="text-xs text-muted-foreground">{tr('50 événements par page, du plus récent au plus ancien.','50 events per page, newest first.','50 حدثًا في الصفحة، بدءًا بالأحدث.')}</p>
...
function VerificationDocuments({id,labels,editable,busy,archive}:{id:number;labels:Record<string,string>;editable:boolean;busy:boolean;archive:(id:number)=>void}) {
  ...
  return <div className="space-y-4">
    ...
    <p className="text-xs text-muted-foreground">{tr('50 pièces par page, du dernier dépôt au plus ancien.','50 files per page, latest upload first.','50 ملفًا في الصفحة، بدءًا بآخر رفع.')}</p>
    {archived && <p className="text-sm text-muted-foreground">{tr('Les pièces archivées ne sont plus utilisées pour la revue. Leurs informations restent consultables ici ; leur téléchargement est désactivé.','Archived evidence is no longer used for review. Its information remains available here; downloads are disabled.','لم تعد المستندات المؤرشفة مستخدمة للمراجعة. تبقى معلوماتها متاحة هنا والتنزيل معطل.')}</p>}
    ...
    {page.isError?<VerificationReadError busy={page.isFetching} retry={()=>{void page.refetch();}}/>:<>
      ...
      {page.data?.entries.map(doc=><div key={doc.id} className="flex flex-wrap gap-3 items-center justify-between border-b py-3">
        <div className="min-w-0 space-y-1"><div className="flex items-center gap-2 text-sm break-all"><FileText size={18}/>{doc.fileUrl?<a href={doc.fileUrl} target="_blank" rel="noreferrer" className="underline">{labels[doc.kind]} · {doc.fileName}</a>:<span>{labels[doc.kind]} · {doc.fileName}</span>}</div>
          <p className="text-xs text-muted-foreground">{tr('Déposé le','Uploaded on','تاريخ الرفع')} {new Date(doc.createdAt).toLocaleString(lang)}</p>
          {doc.archivedAt && <p className="text-xs text-muted-foreground">{tr('Archivé le','Archived on','تاريخ الأرشفة')} {new Date(doc.archivedAt).toLocaleString(lang)}</p>}
        </div>
