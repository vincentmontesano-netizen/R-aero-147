      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl p-8" style={{ background: "var(--background)" }}>
          <div className="flex flex-col items-center mb-6">
            ...
            <h1 className="font-sans text-2xl font-bold" style={{ color: "var(--foreground)" }}>{t("register.title")}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="reg-field-1" className="text-sm font-semibold mb-1.5 block" style={{ color: "var(--muted-foreground)" }}>{t("register.labelName")}</label>
              <Input id="reg-field-1" required value={form.name} onChange={set("name")} placeholder={t("register.placeholderName")} />
            </div>
            <div>
              <label htmlFor="reg-field-2" className="text-sm font-semibold mb-1.5 block" style={{ color: "var(--muted-foreground)" }}>{t("register.labelEmail")}</label>
            ...
            <div>
              <label htmlFor="reg-field-3" className="text-sm font-semibold mb-1.5 block" style={{ color: "var(--muted-foreground)" }}>{t("register.labelPassword")} <span className="font-normal">{t("register.labelPasswordHint")}</span></label>
            ...
            <div className="rounded-lg p-3" style={{ border: "1px solid var(--border)", background: "var(--background)" }}>
              <label className="flex items-center justify-between gap-3 cursor-pointer">
                <span className="text-sm font-medium" style={{ color: "var(--foreground)" }}>{t("register.createOrgToggle")}</span>
                <input type="checkbox" checked={form.createOrg} onChange={(e) => setForm((f) => ({ ...f, createOrg: e.target.checked }))} className="w-4 h-4" />
              </label>
              {form.createOrg && (
                <div className="space-y-2 mt-3">
                  <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>{t("register.createOrgHint")}</p>
                  <div>
                    <label htmlFor="reg-field-4" className="text-sm font-semibold mb-1 block" style={{ color: "var(--muted-foreground)" }}>{t("register.orgNameLabel")}</label>
                    <Input id="reg-field-4" value={form.orgName} onChange={set("orgName")} placeholder="Aero MRO SA" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label htmlFor="reg-field-5" className="text-sm font-semibold mb-1 block" style={{ color: "var(--muted-foreground)" }}>{t("register.orgTypeLabel")}</label>
                      <select id="reg-field-5" value={form.orgType} onChange={(e) => setForm((f) => ({ ...f, orgType: e.target.value }))} className="w-full h-9 rounded-md border px-2 text-sm" style={{ borderColor: "var(--border)" }}>
                    ...
                    <div>
                      <label htmlFor="reg-field-6" className="text-sm font-semibold mb-1 block" style={{ color: "var(--muted-foreground)" }}>{t("register.orgAgreementLabel")}</label>
            ...
            <label className="flex items-start gap-2 text-sm cursor-pointer" style={{ color: "var(--muted-foreground)" }}>
              <input
                type="checkbox"
            ...
            <Button type="submit" disabled={register.isPending} className="w-full" style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>
          ...
          <p className="text-sm text-center mt-6" style={{ color: "var(--muted-foreground)" }}>
            ...
            <Link href="/login">
              <span className="font-semibold cursor-pointer hover:underline" style={{ color: "var(--link)" }}>{t("register.login")}</span>
            </Link>
          </p>
        </div>
      </div>
