    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: DEEP_BLUE }}>
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl p-8" style={{ background: IVORY }}>
          ...
          {!validToken ? (
            <p role="alert" className="text-sm text-center" style={{ color: "oklch(55% 0.22 27)" }}>{t("resetPassword.invalidLink")}</p>
          ) : done ? (
            <p role="status" className="text-sm text-center flex items-center justify-center gap-2" style={{ color: "oklch(45% 0.15 145)" }}><CheckCircle className="w-4 h-4" /> {t("resetPassword.success")}</p>
          ) : (
            <form onSubmit={submit} className="space-y-4" aria-busy={reset.isPending}>
              <div>
                <label htmlFor="new-password" className="text-xs font-semibold mb-1.5 block" style={{ color: "oklch(45% 0.02 240)" }}>{t("resetPassword.newPassword")}</label>
              ...
              <div>
                <label htmlFor="confirm-password" className="text-xs font-semibold mb-1.5 block" style={{ color: "oklch(45% 0.02 240)" }}>{t("resetPassword.confirmPassword")}</label>
              ...
              {errorKey && <p role="alert" className="text-sm" style={{ color: DEEP_BLUE }}>{t(errorKey)}</p>}
