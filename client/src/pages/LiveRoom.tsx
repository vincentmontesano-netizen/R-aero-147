...
function LiveRoomSession({ type, id }: { type?: string; id?: string }) {
  ...
  if (!isAuthenticated) return <Centered><a href={getLoginUrl()}><Button style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>{t("liveRoom.login")}</Button></a></Centered>;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--surface-strong)" }}>
...
function Centered({ children }: { children: any }) {
  return <div className="min-h-screen flex items-center justify-center p-6 text-center text-sm" style={{ background: "var(--background)", color: "var(--muted-foreground)" }}><div className="max-w-sm">{children}</div></div>;
}
...
function SidePanel({ room, replay, userId }: { room: Room; replay: boolean; userId: number }) {
  ...
  return (
    <div className="flex flex-col min-h-0" style={{ background: "var(--card)", borderLeft: `1px solid ${"var(--border)"}` }}>
      <div className="flex shrink-0" style={{ borderBottom: `1px solid ${"var(--border)"}` }}>
        {visibleTabs.map((tabItem) => (
          <button key={tabItem.key} type="button" aria-pressed={activeTab === tabItem.key} onClick={() => setTab(tabItem.key)} className="flex-1 py-2.5 flex items-center justify-center gap-1 text-sm font-semibold transition-colors"
            style={{ color: activeTab === tabItem.key ? "var(--foreground)" : "var(--muted-foreground)", borderBottom: activeTab === tabItem.key ? `2px solid ${"var(--link)"}` : "2px solid transparent" }}>
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1">
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
        ...
        {!messagesQuery.isLoading && !messagesQuery.isError && list.length === 0 && <p className="text-xs text-center mt-6" style={{ color: "var(--muted-foreground)" }}>{kind === "chat" ? t("liveRoom.noMessages") : t("liveRoom.noQuestions")}</p>}
        {list.map((m: any) => (
          <div key={m.id} className="text-sm">
            <span className="font-semibold" style={{ color: "var(--foreground)" }}>{m.authorName}</span>
            {kind === "qa" && m.isAnswered && <span className="ml-1 text-xs px-1.5 rounded-full" style={{ background: "color-mix(in srgb, var(--success) 15%, transparent)", color: "var(--success)" }}>{t("liveRoom.answered")}</span>}
            <div style={{ color: "var(--muted-foreground)" }}>{m.content}</div>
            {kind === "qa" && room.isModerator && !m.isAnswered && (
              <button disabled={markAnswered.isPending || messagesQuery.isError} onClick={() => markAnswered.mutate({ messageId: m.id })} className="text-sm mt-0.5 inline-flex items-center gap-1 disabled:opacity-50" style={{ color: "var(--link)" }}><Check className="w-3 h-3" /> {t("liveRoom.markAnswered")}</button>
            )}
            {markAnswered.isError && markAnswered.variables?.messageId === m.id && <p role="alert" className="text-xs text-destructive">{markAnswered.error.message}</p>}
          </div>
        ))}
      </div>
      {postUnconfirmed && <p role="alert" className="p-2 text-xs text-destructive">{t('liveRoom.messageUnconfirmed')}</p>}
      <form className="p-2 flex gap-2 shrink-0" style={{ borderTop: `1px solid ${"var(--border)"}` }}
        onSubmit={(e) => {
          e.preventDefault();
        ...
        <Button type="submit" size="sm" aria-label={t('liveRoom.sendMessage')} disabled={!text.trim() || post.isPending || messagesQuery.isError} style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}><Send aria-hidden="true" className="w-4 h-4" /></Button>
      </form>
    </div>
  );
}
...
function ParticipantsPanel({ room }: { room: Room }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
      ...
      {!peopleQuery.isLoading && !peopleQuery.isError && peopleQuery.data?.length === 0 && <p className="text-xs text-center mt-6" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.noParticipants")}</p>}
      {!peopleQuery.isError && peopleQuery.data?.map(p => (
        <div key={p.userId} className="flex items-center gap-2 text-sm">
          <span aria-hidden="true" className="w-2 h-2 rounded-full shrink-0" style={{ background: p.online ? "color-mix(in srgb, var(--success) 18%, transparent)" : "var(--muted)" }} />
          <span className="min-w-0 break-words" style={{ color: "var(--foreground)" }}>{p.name}</span>
          <span className="text-xs ml-auto" style={{ color: "var(--muted-foreground)" }}>{t(p.online ? 'liveRoom.presenceRecent' : 'liveRoom.presenceUnconfirmed')}</span>
        </div>
      ))}
    </div>
  );
...
function EngagementPanel({ room }: { room: Room }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3">
      <p className="text-xs mb-2" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.scoreFormula")}</p>
      <p className="text-xs mb-2" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.presenceEstimate")}</p>
      ...
      {!scoresQuery.isLoading && !scoresQuery.isError && scoresQuery.data?.length === 0 && <p className="text-xs text-center mt-6" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.noData")}</p>}
      {!scoresQuery.isError && scoresQuery.data?.map((s, i) => (
        <div key={s.userId} className="flex items-center gap-2 p-2 rounded-lg mb-1" style={{ background: "var(--background)" }}>
          <span className="w-5 text-xs font-bold" style={{ color: "var(--link)" }}>{i + 1}</span>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate" style={{ color: "var(--foreground)" }}>{s.name}</div>
            <div className="text-xs" style={{ color: "var(--muted-foreground)" }}>{s.attendanceMin}min · {s.chat}💬 · {s.qa}❓ · {s.votes}🗳 · {s.quizCorrect}✓</div>
          </div>
          <span className="font-sans font-bold" style={{ color: "var(--foreground)" }}>{s.score}</span>
        </div>
      ))}
    </div>
  );
...
function PollsPanel({ room }: { room: Room }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-3">
      ...
      {!pollQuery.isLoading && !pollQuery.isError && polls.length === 0 && <p className="text-xs text-center mt-4" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.noPolls")}</p>}
      {polls.map((p: any) => {
        const total = p.totalVotes || 0;
        const showResults = room.isModerator || p.kind !== 'quiz' || !p.isOpen;
        return (
          <div key={p.id} className="rounded-lg p-3" style={{ background: "var(--background)" }}>
            <div className="flex items-center justify-between gap-2 mb-2">
              <span className="text-sm font-semibold" style={{ color: "var(--foreground)" }}>{p.kind === "quiz" ? "🎯 " : "📊 "}{p.question}</span>
              {!p.isOpen && <span className="text-xs" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.closed")}</span>}
            </div>
              ...
              {(p.options ?? []).map((opt: string, i: number) => {
                ...
                return (
                  <button key={i} type="button" aria-pressed={(p.myChoices ?? []).includes(i)} disabled={!p.isOpen || vote.isPending || pollQuery.isError} onClick={() => vote.mutate({ pollId: p.id, choices: [i] })}
                    className="w-full text-left rounded-md overflow-hidden relative" style={{ border: `1px solid ${"var(--border)"}` }}>
                    <div className="absolute inset-y-0 left-0" style={{ width: `${pct}%`, background: isCorrect ? "color-mix(in srgb, var(--success) 18%, transparent)" : "color-mix(in srgb, var(--link) 18%, transparent)" }} />
                    <div className="relative flex justify-between px-2.5 py-1.5 text-sm" style={{ color: "var(--foreground)" }}>
                      <span>{String.fromCharCode(65 + i)}. {opt}{isCorrect ? " ✓" : ""}{isCorrect && <span className="sr-only"> {t('liveRoom.correctAnswer')}</span>}</span>
                      {showResults && <span className="text-xs" style={{ color: "var(--muted-foreground)" }}>{pct}% ({count})</span>}
                    </div>
                  </button>
                );
              })}
            ...
            {((vote.isError && vote.variables?.pollId === p.id) || (close.isError && close.variables?.pollId === p.id)) && <p role="alert" className="text-xs text-destructive mt-2">{t('liveRoom.pollActionUnconfirmed')}</p>}
            {room.isModerator && p.isOpen && <button type="button" disabled={close.isPending || pollQuery.isError} onClick={() => close.mutate({ pollId: p.id })} className="text-sm mt-2 disabled:opacity-50" style={{ color: "var(--muted-foreground)" }}>{t("liveRoom.closeVote")}</button>}
          </div>
        );
      })}
    </div>
    ...
    <div className="rounded-lg p-3 space-y-2" style={{ background: "var(--background)", border: `1px solid ${"var(--border)"}` }}>
      <div className="flex gap-2">
        {(["poll", "quiz"] as const).map((k) => (
          <button key={k} onClick={() => setKind(k)} className="flex-1 py-1 rounded text-sm font-semibold" style={{ background: kind === k ? "var(--primary)" : "var(--card)", color: kind === k ? "var(--primary-foreground)" : "var(--muted-foreground)", border: `1px solid ${"var(--border)"}` }}>{k === "poll" ? t("liveRoom.kindPoll") : t("liveRoom.kindQuiz")}</button>
        ))}
      </div>
      ...
      {options.map((o, i) => (
        <div key={i} className="flex items-center gap-2">
          {kind === "quiz" && <button type="button" aria-pressed={correct.includes(i)} aria-label={`${t('liveRoom.correctAnswer')} ${i + 1}`} onClick={() => setCorrect([i])} title={t("liveRoom.correctAnswer")} className="w-4 h-4 rounded-full border shrink-0" style={{ borderColor: correct.includes(i) ? "var(--success)" : "var(--border)", background: correct.includes(i) ? "color-mix(in srgb, var(--success) 18%, transparent)" : "transparent" }} />}
      ...
      <div className="flex items-center justify-between">
        <button type="button" disabled={options.length >= 10} onClick={() => setOptions((s) => [...s, ""])} className="text-sm disabled:opacity-50" style={{ color: "var(--link)" }}>{t("liveRoom.addOption")}</button>
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={onDone}>{t("liveRoom.cancel")}</Button>
          <Button size="sm" disabled={create.isPending} onClick={submit} style={{ background: "var(--primary)", color: "var(--primary-foreground)" }}>{t("liveRoom.launch")}</Button>
        </div>
      </div>
    </div>
  );
