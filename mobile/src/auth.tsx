...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const accept = async (
    result: Learner & { nativeSession?: { token: string; expiresAt: string } }
  ) => {
    if (!result.nativeSession?.token)
      throw new Error(
        "La connexion mobile n’est pas disponible sur ce serveur."
      );
    ...
    generation.current++;
    setSessionToken(nativeSession.token);
    setUser(profile as Learner);
    setRestoreError(null);
  };
  ...
  const retry = useCallback(async () => {
    ...
    try {
      ...
      if (active === generation.current)
        setRestoreError(
          "Impossible de retrouver votre session. Vérifiez votre connexion puis réessayez."
        );
    } finally {
      setRestoring(false);
    }
    ...
    () =>
      queryClient.getQueryCache().subscribe(event => {
        if (
          ...
          (event.query.state.error as any)?.data?.code === "UNAUTHORIZED" &&
          sessionToken()
        )
          void signOut();
      }),
    ...
    () =>
      queryClient.getMutationCache().subscribe(event => {
        if (
          event.type === "updated" &&
          event.action.type === "error" &&
          (event.mutation.state.error as any)?.data?.code === "UNAUTHORIZED" &&
          sessionToken()
        )
          void signOut();
      }),
  ...
  const verify = async (email: string, code: string) =>
    accept(
      await apiClient.auth.verifyTwoFactor.mutate({
        email: email.trim().toLowerCase(),
        code,
      })
    );
  const register = async (name: string, email: string, password: string) =>
    accept(
      await apiClient.auth.register.mutate({
        name,
        email,
        password,
