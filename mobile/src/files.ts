...
export async function clearTemporaryExports() {
...
export async function sharePdf(url: string, title: string) {
  ...
  if (!(await Sharing.isAvailableAsync()))
    throw new Error(
      "Le partage de documents est indisponible sur cet appareil."
    );
  ...
  try {
    ...
    if (result.status !== 200)
      throw new Error(
        "Le document n’a pas pu être téléchargé. Vérifiez votre accès et réessayez."
      );
    ...
    if (prefix !== "JVBERi0=")
      throw new Error("Le serveur n’a pas fourni un document PDF valide.");
    if (credential !== sessionToken())
      throw new Error(
        "Votre session a changé. Rouvrez le document depuis votre compte."
      );
